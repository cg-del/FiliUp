-- Make content column nullable in activities table
...
-- Create new tables for activity content
...
CREATE TABLE IF NOT EXISTS drag_drop_items (
    ...
    order_index INTEGER NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_dragdrop_item_activity FOREIGN KEY (activity_id) REFERENCES activities(id)
);
...
CREATE TABLE IF NOT EXISTS drag_drop_categories (
    ...
    order_index INTEGER NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_dragdrop_category_activity FOREIGN KEY (activity_id) REFERENCES activities(id)
);
...
CREATE TABLE IF NOT EXISTS matching_pairs (
    ...
    order_index INTEGER NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_matching_pair_activity FOREIGN KEY (activity_id) REFERENCES activities(id)
);
...
CREATE TABLE IF NOT EXISTS lesson_slides (
    ...
    order_index INTEGER NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_lesson_slide_lesson FOREIGN KEY (lesson_id) REFERENCES lessons(id)
);
